...
const SideBar = () => {
    ...
    return (
        <div className='flex h-screen'>
            <div className="sidebar w-1/5 flex flex-col gap-3 py-10 px-2 border-r-4">
                ...
                <nav className="navlinks w-full flex flex-col gap-10 pl-10 pt-10 mx-auto items-start">
                    {navlinks.map((link,key)=>(
                        <NavLink key={key} to={link.href} className={`${(isActive:boolean)=>isActive?'text-primary':'text-black'} hover:text-blue-600 text-xl flex gap-3 items-center`} >
                            {link?.icon} {link.text}
                        </NavLink>
                    ))}
                </nav>
